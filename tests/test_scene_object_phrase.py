...
class TestSceneObjectPhrase(unittest.TestCase):
    ...
    def test_scene_object_phrase_repr(self):
        ...
        np = NounPhrase()
        noun_token = VectorSpace(word="sphere")
        noun_token["noun"] = 1.0
        np.apply_noun(noun_token)
        ...
        # After resolution
        scene_obj = SceneObject("test_sphere", {})
        so.resolve_to_scene_object(scene_obj)
...
class TestLayer2GroundingIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Create a red cube
        self.red_cube = SceneObject("red_cube", {})
        self.red_cube.vector = VectorSpace()
        self.red_cube.vector["noun"] = 1.0
        self.red_cube.vector["red"] = 1.0
        self.red_cube.vector["singular"] = 1.0
        self.scene.add_object(self.red_cube)
        
        # Create a blue sphere
        self.blue_sphere = SceneObject("blue_sphere", {})
        self.blue_sphere.vector = VectorSpace()
        self.blue_sphere.vector["noun"] = 1.0
        self.blue_sphere.vector["blue"] = 1.0
        self.blue_sphere.vector["singular"] = 1.0
    ...
    def test_layer2_grounding_creates_scene_object_phrase(self):
        ...
        # Verify results
        self.assertTrue(result.success, "Grounding should succeed")
    ...
    def test_layer2_grounding_multiple_candidates(self):
        ...
        np = NounPhrase()
        noun_token = VectorSpace(word="object")
        noun_token["noun"] = 1.0
        noun_token["singular"] = 1.0
        np.apply_noun(noun_token)
    ...
    def test_layer2_grounding_failure(self):
        ...
        np = NounPhrase()
        noun_token = VectorSpace(word="triangle")
        noun_token["noun"] = 1.0
        noun_token["singular"] = 1.0
        np.apply_noun(noun_token)
