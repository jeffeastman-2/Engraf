...
EngrafVPPolicy's final `return False` rejects every verb phrase that
...
existing 265 latn tests (ground_layer4 runs them through EngrafVPPolicy).
"""

from engraf.lexer.hypothesis import TokenizationHypothesis
from engraf.lexer.semantic_grounding_layer4 import Layer4SemanticGrounder
from engraf.lexer.vector_space import vector_from_features
from engraf.lexer.vp_policy import (
    EngrafVPPolicy,
    PermissiveVPPolicy,
    VPGroundingPolicy,
...
def test_policies_satisfy_protocol():
    assert isinstance(EngrafVPPolicy(), VPGroundingPolicy)
...
def test_engraf_default_is_fail_closed_permissive_is_not():
    ...
    vp = _non_cad_vp()
    assert not EngrafVPPolicy().validate_vp(vp)
